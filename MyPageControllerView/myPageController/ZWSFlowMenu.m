...
//        x = (CGRectGetWidth(self.frame) - w) / 2.0 + _menuInsets.left;
    ...
    for (ZWSMenuLabel *v in _items) {
        ...
        v.tagImage.center = CGPointMake(x + CGRectGetWidth(v.frame) + (CGRectGetWidth(v.tagImage.frame)/2.0), CGRectGetMinY(v.frame));
        [self addSubview:v];
        [self insertSubview:v.tagImage aboveSubview:v];
